        ...
        <Panel name="1">
          ...
          <div slot="content" class="security-select">
            <Form :label-width="100">
              <FormItem label="安全级别：">
                <Select style="width:400px" v-model="levelSelect" @on-open-change="openSafe" @on-change="onSafe" placeholder="请选择安全级别">
                  <Option v-for="item in securityLevel" :value="item.id" :key="item.id">{{ item.level }}</Option>
                </Select>
              </FormItem>
            </Form>
          </div>
          <div slot="content" class="password">
            <span class="_title">设置密码</span>
            <Form :model="formItem" :label-width="100" ref="formItem" :rules="validateRules">
              <FormItem label="原密码：">
                  <Input  type="text" v-model="formItem.old" placeholder="请输入原密码" style="width: 70%"></Input>
              </FormItem>
               <FormItem label="新密码：" prop="newPassword">
                  <Input  type="password" v-model="formItem.newPassword" placeholder="请输入新密码" style="width: 70%"></Input>
              </FormItem>
               <FormItem label="重输密码：" prop="repeatPwd">
                  <Input  type="password" v-model="formItem.repeatPwd" placeholder="重新输入新密码" style="width: 70%"></Input>
              </FormItem>
            </Form>
      ...
      <!-- 系统参数设置 -->
        <Panel name="2">
          ...
          <div slot="content">
            ...
            <Button @click="onSystem" type="success">保存</Button>
          </div>
        </Panel>
              ...
              <Col span="12">
                <!-- <div slot="content" class="Email"> -->
                  ...
                  <Form :label-width="160">
                    <FormItem label="邮 件 账 户：">
                        <Input type="text" v-model="mailbox.admin" placeholder="请输入邮件账户" style="width: 70%"></Input>
                    </FormItem>
                    <FormItem label="邮 件 密 码：">
                        <Input type="password" v-model="mailbox.password" placeholder="请输入邮件密码" style="width: 70%"></Input>
                    </FormItem>
                    <FormItem label="SMTP服务器地址：">
                        <Input v-model="mailbox.path" placeholder="重新输入SMTP服务器地址" style="width: 70%"></Input>
                    </FormItem>
                    <FormItem label="SMTP服务器端口号：">
                        <Input v-model="mailbox.port" placeholder="重新输入SMTP服务器端口号" style="width: 70%"></Input>
                    </FormItem>
                  </Form>
                  <Button class="email-but" @click="onMailbox" type="success">&nbsp;保 存&nbsp;</Button>
                <!-- </div> -->
              </Col>
            <!-- 右侧 -->
              <Col span="12">
                EMAIL地址：
                <Input v-model="mailbox.address" placeholder="请输入EMAIL地址" style="width: 70%"></Input>
                <div class="_button">
                  <Button type="success" @click="addEmail">添加</Button>
                  <Button type="warning" @click="preservation">保存</Button>
                  <Button type="error" @click="deleteEmail">删除</Button>
                </div>
                <Table class="_tabs" highlight-row border :columns="emails" :data="emailData" @on-row-click="rowClick" height="200"></Table>
              </Col>
          </Row>
        </Panel>
    </Collapse>
  ...
  export default {
    data () {
      const pwdValidate = (rule, value, callback) => {
        this.$refs.formItem.validateField('repeatPwd');
        callback();
      };
      const pwdCheckValidate = (rule, value, callback) => {
        if (this.formItem.newPassword != '' && value == '') {
            callback(new Error('确认密码不能为空'));
        } else if (this.formItem.newPassword != value) {
    ...
    created() {
      util.restfullCall('/rest-ful/v3.0/system/param', null, 'get', this.systemList)
    },
    methods: {
      // 点击下拉框获取下拉框信息
      openSafe:function(open) {
         this.securityLevel = [{ id: "3", level: "高" },{ id: "2", level: "中" },{ id: "1", level:"低" }]
      },
      // 级别下拉框狂id
      onSafe:function(id) {
        util.restfullCall('/rest-ful/v3.0/system/param', [{ type: 1,value:id }] , 'put', this.onSafeData)
      },
      // 级别下拉框id回调
      ...
      col1:function (ca) {
        if(ca[0]==3) {
          util.restfullCall('/rest-ful/v3.0/system/emails', null, 'get', this.emailsData)
        }
        return false;
      },
      // 系统参数接口回调
      ...
      confirm:function() {
        ...
        util.restfullCall('/rest-ful/v3.0/usr/password/'+user.uid, { oldpassword:this.formItem.old,newpassword:this.formItem.newPassword }, 'put', this.passwordData)
      },
      // 设置密码确认修改回调
      ...
      // 保存系统参数
      onSystem:function() {
        util.restfullCall('/rest-ful/v3.0/system/param',
      ...
      // 添加Email到表格
      addEmail:function() {
        util.restfullCall('/rest-ful/v3.0/system/email', {email:this.mailbox.address}, 'POST', this.accountData)
      },
      // 添加Email到表格的回调
      ...
      rowClick:function(row) {
        ...
        this.mailbox.address = row.email
      },
      // 点击保存按钮保存修改后的数据
      preservation:function() {
        util.restfullCall('/rest-ful/v3.0/system/email', {id:this.mailbox.addressID,email:this.mailbox.address}, 'PUT', this.calbakData)
      },
      // 保存修改的数据回调
      ...
      deleteEmail:function() {
        util.restfullCall('/rest-ful/v3.0/system/email/'+this.mailbox.addressID, null , 'delete', this.emailDelete)
      },
      // 删除Email表格数据回调
