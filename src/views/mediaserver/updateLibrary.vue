...
<template>
  ...
  >
    ...
    <div class="parent" style="margin-bottom:20px">
      <div
        v-for="(value,key) in changer"
        :key="key"
        v-if="(typeof(value) !== 'object') && (key !== 'id')"
      >{{ralations[key]}}: {{value}}</div>
    </div>
...
export default {
          ...
          render: (h, params) => {
            return h("div", [
              ...
              h("Icon", {
                ...
                on: {
                  click: () => {
                    if (confirm("是否确认将磁带从该驱动器中移出？")) {
                      util.restfullCall(
                        "/rest-ful/v3.0/tape/unload?device=" +
                          this.libraryList.id +
                          "&driver=" +
                          params.row.driverno,
                        null,
                    ...
                    h(
                      "DropdownItem",
                      {
                        nativeOn: {
                          click: () => {
                            util.restfullCall(
                              "/rest-ful/v3.0/tape/load?device=" +
                                this.libraryList.id +
                                "&driver=" +
                                item.driverno +
                                "&slot=" +
  ...
  methods: {
    ...
    livrayModify: function(row) {
      ...
      // 获取磁盘详情列表数据
      util.restfullCall(
        "/rest-ful/v3.0/mediumchanger/" + row.changer,
        null,
        "get",
        this.address
      );
