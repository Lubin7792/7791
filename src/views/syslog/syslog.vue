    ...
    <TabPane label="系统事件">
       <div class="sys-top">
        <Row type="flex" justify="space-between">
            <Col span="4">
              <span>级别:</span>
                <Select v-model="refreshEvent.level" clearable @on-change="changesEvent" @on-open-change="eventLevel" style="width:130px">
                  <Option v-for="item in levelEvent" :value="item.level" :key="item.level">{{ item.name }}</Option>
                </Select>
            </Col>
            <Col span="4">
              <span>来源:</span>
                <Select v-model="refreshEvent.source" clearable @on-change="onSource" @on-open-change="eventSource" style="width:130px">
                  <Option v-for="item in sourceEvent" :value="item.module" :key="item.module">{{ item.name }}</Option>
                </Select>
            </Col>
            <Col span="5">
                <span>开始时间</span>
                <DatePicker type="date" placeholder="请选择开始日期" style="width: 170px" @on-change="startEvent"></DatePicker>
            </Col>
            <Col span="5">
              <span>结束时间</span>
...
import sysdetails from './sysdetails.vue'
export default {
  components: {
    syscontent,
    sysdetails
...
// 系统事件
    // 级别下拉框触发事件
    eventLevel:function(open) {
      if(open == true) util.restfullCall('/rest-ful/v3.0/loglevels', null, 'get', this.eventData)
    },
    // 级别下拉框回调
    eventData:function(obj) {
      var array = new Array
      for(let i = 0;i < obj.data.length;i++ ){
        array.push({
          level:obj.data[i].level,
    ...
    // 级别下拉框选中的id
    changesEvent:function(id) {
      this.refreshEvent.level = id
    },
    // 来源下拉框生成的数据
    eventSource:function(open) {
      if(open == true) util.restfullCall('/rest-ful/v3.0/logmodules', null, 'get', this.callbackSourcel)
    },
    // 来源下拉框回调数据
    callbackSourcel:function(obj) {
      var array = new Array
      for(let i = 0;i < obj.data.length;i++ ){
        array.push({
          module:obj.data[i].module,
    ...
    // 来源下拉框选中的id
    onSource:function(id) {
      this.refreshEvent.source = id
    },
    // 开始时间
    startEvent: function(start) {
      this.refreshEvent.starttime = start
    },
    // 结束时间
    endEvent: function(end) {
      this.refreshEvent.endtime = end
    },
    // 点击查询触发系统事件
    onEvent() {
      let url = '/rest-ful/v3.0/sysevents?'
      Object.keys(this.refreshEvent).forEach(item => {
        if(this.refreshEvent[item]) url += `${item}=${this.refreshEvent[item]}&`
      })
      util.restfullCall(url, null, 'get', this.callbackEvent)
    },
    // 刷新数据之后的回调传递给子组件表格
    callbackEvent: function(obj) {
      ...
      this.$refs.sysdetails.queryEvent(array)
    },
  },
}
</script>